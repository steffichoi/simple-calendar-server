...
char * list_calendars(Calendar *cal_list) {
    Calendar *current = cal_list;
    int list_size = 0;  //keep track of how much space is needed for the string
    
    //loop through once to add up the space needed for the string
    while (current){
        list_size += sizeof(current->name) + 2;
        current = current->next;
    }
    
    current = cal_list;  //reset the pointer to the head of the list
    
    char * list;  //make a char * to store the results as a string
    if (!(list = malloc(list_size))){  //error check for malloc
        perror("malloc failed!\n");
        exit(1);
    }
    
    //loop through a second time, concatenating calendar names to the string
    while (current) {
        char * newline = "\r\n";  //add a newline character after every name
        
        printf("list loop\n");
        
        //concatenate the calendar name to the string
        strcat(list, current->name);
        //concatenate a newlinen characted after the name
        strcat(list, newline);
        
        current = current->next;
    }
    
    printf("list: %s", list);  //testing purposes
    return list;
...
char * list_users(User *user_list) {
    ...
    current = user_list;  //reset the pointer to the head of the list
...
char * list_events(Calendar *cal_list, char *cal_name) {
    ...
    char * err = "-1";  //allocate a char * so that the return type will be char *
    ...
    list_size += sizeof(this_cal->name);
    
    //loop through once to add up the space needed for the string
    while (cur) {
        list_size += sizeof(cur->description);
        list_size += sizeof(ctime(&(cur->time)));
        cur = cur->next;
    }
    
    cur = this_cal->events;  //reset the pointer to the head of the list
    ...
    char * list_start = "Events for calendar ";
    char * first_line = ":\r\n";
    strcat(list, list_start);
    strcat(list, this_cal->name);
    strcat(list, first_line);
    
    while (cur) {
        char * colon = ":";
        
        //concatenate the event description to the string
        strcat(list, cur->description);
        //concatenate the colon to separate the event description and its time
        strcat(list, colon);
        //concatenate the event's time to the string
        strcat(list, ctime(&(cur->time)));
        
        cur = cur->next;
    }
    
//    printf("%s", list);  //testing purposes
    return list;
}
